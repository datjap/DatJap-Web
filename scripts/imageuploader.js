...
function btnClick() {
    ...
    if (fileSave !== undefined) {
        var selectSize = sizers[1].x - sizers[0].x;
        ...
        $.ajax({
                ...
                data: {
                    ...
                    image: fileSave,
                    zoom: zoom,
                    x: round(map(sizers[0].x, 0, (img.width * multiplier) - selectSize, 0, 100)),
                    y: round(map(sizers[0].y, 0, (img.height * multiplier) - selectSize, 0, 100))
                }
            })
            .done(function() {
